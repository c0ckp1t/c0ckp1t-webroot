<script setup>
import {computed, defineProps, reactive, onMounted, inject} from 'vue'
...
// Injected from parent XMarkdown component via provide/inject
const fetchText = inject('fetchText', async (url) => ({ isOk: false, result: `No fetchText provider for ${url}` }))

// ________________________________________________________________________________
// LOGGING
// ________________________________________________________________________________
const LOG_HEADER = 'code-item2.vue'
...
async function findCode() {
  ...
  const resp = await fetchText( props.url)
  logger.debug(resp)
  if (!resp.isOk) {
