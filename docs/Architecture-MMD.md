...
block-beta
  ...
  browser -- "HTTP / WebSocket (msgpack)" --> server
...
sequenceDiagram
    ...
    HTML->>Config: import config
    Config->>CU: (imports defaultVueComponents / findHostnamePortProtocol)
    Config-->>HTML: config object (incl. config.islands)
    ...
    GS->>GS: for each config.islands → api.registerIsland(island)
    activate GS
...
1. **`index.html`** loads CSS (Bootstrap, FontAwesome, highlight.js, KaTeX), defines the import map, and imports the single app config from `config.mjs`.
...
```mermaid
flowchart TD
    REQ["SFC Loader requests file"]
    REQ --> CACHE{"In sourceCode\ncache?"}
    CACHE -->|Yes| RET_CACHE["Return cached source"]
    CACHE -->|No| HTTP{"Starts with\nhttp:// ?"}
    HTTP -->|Yes| FETCH["Default island\nfetch(path)"]
    HTTP -->|No| INST{"Starts with\n/instanceId/ ?"}
    INST -->|Yes| ISLAND["Island resolver()\n(HTTP or WebSocket RPC)"]
    INST -->|No| DEFAULT["Default island\nresolver(path)"]

    FETCH --> RESULT["Return file content"]
    ISLAND --> RESULT
    DEFAULT --> RESULT
```
...
sequenceDiagram
    ...
    Note over C,S: exec() — Request/Response
    C->>S: EXEC_REQ { id, endpoint, args }
    S->>C: EXEC_RESP { id, bytes }

    Note over C,S: exec2() — Server Streaming
    C->>S: EXEC2_REQ { id, endpoint, args }
    S->>C: ACCEPT { id }
    loop Stream
        S->>C: EXEC2_RESP { id, bytes }
    end
    S->>C: COMPLETE { id }

    Note over C,S: exec3() — Bidirectional Streaming
    C->>S: EXEC3_REQ { id, endpoint, args }
    S->>C: ACCEPT { id }
    par Client pushes
        C->>S: EXEC3_PUSH { id, bytes }
    and Server responds
        S->>C: EXEC3_RESP { id, bytes }
    end
    C->>S: EXEC3_CLOSE { id }
    S->>C: COMPLETE { id }
```
...
stateDiagram-v2
    ...
    IDLE --> FETCHING_COOKIE : connect

    FETCHING_COOKIE --> CONNECTING_WS : onDone
    FETCHING_COOKIE --> FAILED : onError

    CONNECTING_WS --> CONNECTED : onDone
    CONNECTING_WS --> FAILED : ws.error / ws.closed / onError

    CONNECTED --> READY : always (transient)

    READY --> DISCONNECTED : disconnect / ws.closed / ws.error

    DISCONNECTED --> FETCHING_COOKIE : connect

    FAILED --> FETCHING_COOKIE : connect (retry)
```
...
```mermaid
flowchart LR
    subgraph Stores["Reactive Stores (Vue.reactive)"]
        GS["GlobalStore.store\napp, router, config\nr (island registry)\nselectedInstId, isReady"]
        IS["Island.store\nroot (nav tree)\nselectedNode\ncontext"]
        ISS["Island.state\nisReady, isLoading\nworkflowTable"]
        CS["Connection.state\ncurrentState\nisConnected\nerror, retries"]
        TS["Theme.store\ntheme, mode\ncolors"]
        NS["NotifyUtils.store\nnotifications[]"]
    end

    subgraph Components["Vue Components"]
        PM["PageMain.vue"]
        PN["PageNavigation.vue"]
        USR["User Pages"]
    end

    Components -->|"import { store }"| Stores
```
...
```mermaid
flowchart TB
    subgraph PageMain["PageMain.vue"]
        direction TB
        OC["MainOffCanvas\n(Sidebar navigation)"]
        NAV["PageNavigation\n(Top navbar)"]

        subgraph Main["main.container-fluid"]
            FB["PageFallback\n(Error display)"]
            RV["RouterView\n(Page content)"]
        end
...
graph TD
    ...
    HTML --> CONFIG
    HTML --> LOG["Logging.mjs"]
    HTML --> GS["GlobalStore.mjs"]

    CONFIG --> CU["ConfigUtils.mjs"]

    GS --> VU["VueUtils.mjs"]
    GS --> CU
    GS --> ID["IslandDefault.mjs"]
    GS --> STASH["Stash.mjs"]
    GS --> JU["JsUtils.mjs"]

    VU --> SFC["vue3-sfc-loader"]
    VU --> GS

    ID --> WU["WsUtils.mjs"]
    ID --> JU

    IS["Island.mjs"] --> CONN["Connection.mjs"]
    IS --> WU
    IS --> CU
    IS --> JU
    IS --> WLOG["WsLogUtils.mjs"]

    CONN --> WC["WsClient.mjs"]
    CONN --> AUTH["AuthNState.mjs"]
    CONN --> WU
    CONN --> RXJS["RxJS"]
    CONN --> XST["XState"]
    ...
    WU --> MSGPACK["msgpack"]
    WU --> MITT["mitt"]
    WU --> NU["NotifyUtils.mjs"]

    STASH --> IDB["idb-keyval"]
